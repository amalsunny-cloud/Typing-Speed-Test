...
function countErrors(input: string, expected: string): number {
    let errors = 0;
...
function updateHighlight(input: string, expected: string) {
...
function calculateWPM(input: string, expected: string, timeTaken: number) {
    if (timeTaken === 0) return 0; // Prevent division by zero
